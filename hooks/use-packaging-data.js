...
export function usePackagingData() {
  ...
  const loadData = async () => {
    try {
      ...
      logger.error("Error loading packaging data:", err.message);
  ...
  const getFilterOptions = () => {
    ...
    const allMaterials = [
      ...new Set(
        products.map((p) => p.specifications?.material).filter(Boolean)
      ),
    ];
    const allSizes = [
      ...new Set(products.map((p) => p.specifications?.size).filter(Boolean)),
    ];
    const allColors = [
