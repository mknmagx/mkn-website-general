...
export function useClaude() {
  ...
  const sendMessage = useCallback(async (message, options = {}) => {
    ...
    try {
      ...
      console.error('Claude hook error:', err);
   ...
   */
  const analyzeText = useCallback(async (text, analysisType = 'general') => {
   ...
   * @returns {Promise<string>} Claude's response
   */
  const chatConversation = useCallback(async (conversation, systemPrompt = null) => {
   ...
   */
  const generateSEOContent = useCallback(async (content, options = {}) => {
    const {
      targetKeywords = [],
   ...
   */
  const translateContent = useCallback(async (text, targetLanguage, sourceLanguage = 'auto') => {
