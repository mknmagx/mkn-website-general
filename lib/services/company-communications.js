...
export const createCommunication = async (communicationData) => {
  try {
    ...
    await checkAndUpdateCompanyStatus(
...
/**
 * Tüm iletişim geçmişini getir (tüm firmalar)
 */
export const getAllCommunications = async (limit = 100) => {
  try {
    const communications = await getDocuments(COMMUNICATIONS_COLLECTION, {
      orderBy: ["createdAt", "desc"],
      limit: limit,
  ...
  try {
    ...
    if (newStatus !== company.status) {
      ...
      await createCommunication({
        ...
        content: `Firma durumu "${getStatusText(
          company.status
        )}" durumundan "${getStatusText(
          newStatus
        )}" durumuna otomatik olarak güncellendi.`,
...
export const getCompanyStats = async (companyId) => {
  try {
    ...
    communications.forEach((comm) => {
      ...
      // Aylık aktivite
      const month = new Date(comm.createdAt?.toDate?.() || comm.createdAt)
        .toISOString()
        .substring(0, 7);
