 ...
 */
const sendEmailViaOutlook = async ({ to, subject, body, recipientName, senderName, conversationId, inReplyTo }) => {
  try {
...
export const createConversation = async (conversationData) => {
  try {
    ...
    const conversation = {
      ...
      // Sayaçlar
      messageCount: 1, // İlk mesaj dahil
      unreadCount: 1,
...
export const getInboxConversations = async (options = {}) => {
  try {
    ...
    // channelMetadata.originalCreatedAt'e göre sırala (eski sistem verileri için)
    // Eğer originalCreatedAt yoksa createdAt kullan
...
export const addMessage = async (conversationId, messageData) => {
  try {
    ...
    // ==========================================================================
    // 🔒 ENHANCED DUPLICATE CHECK - Tüm email mesajlarını kontrol et
    // ==========================================================================
    ...
    if (outlookMessageId || internetMessageId) {
      // Önce bu conversation'daki mesajları kontrol et
      const sameCOnvQuery = query(
        collection(db, COLLECTIONS.MESSAGES),
        where('conversationId', '==', conversationId),
        where('channel', '==', CHANNEL.EMAIL)
      );
      const sameConvMsgs = await getDocs(sameCOnvQuery);
      
      for (const docSnap of sameConvMsgs.docs) {
        const data = docSnap.data();
        if (
          ...
          console.log(`[CRM] ⚠️ DUPLICATE PREVENTED - Message already exists in conversation: ${outlookMessageId || internetMessageId}`);
          return { id: docSnap.id, ...data, skipped: true, skipReason: 'duplicate_in_conversation' };
        }
      }
      
      // Sonra TÜM mesajlarda kontrol et (başka conversation'a eklenmişse)
      const allEmailMsgsQuery = query(
        collection(db, COLLECTIONS.MESSAGES),
        where('channel', '==', CHANNEL.EMAIL)
      );
      const allEmailMsgs = await getDocs(allEmailMsgsQuery);
      
      for (const docSnap of allEmailMsgs.docs) {
        const data = docSnap.data();
        if (
          (outlookMessageId && data.emailMetadata?.outlookMessageId === outlookMessageId) ||
          (internetMessageId && data.emailMetadata?.internetMessageId === internetMessageId)
        ) {
          console.log(`[CRM] ⚠️ DUPLICATE PREVENTED - Message already exists in another conversation (${data.conversationId}): ${outlookMessageId || internetMessageId}`);
          return { id: docSnap.id, ...data, skipped: true, skipReason: 'duplicate_in_other_conversation' };
        }
      }
    }
...
export const approveAndSendMessage = async (conversationId, messageId, userId = null, sendOptions = {}) => {
  try {
    const { channels = ['manual'], recipientEmail, recipientName, recipientPhone, subject } = sendOptions;
    ...
    // WhatsApp gönderimi (şimdilik placeholder)
    if (channels.includes('whatsapp') && recipientPhone) {
      // TODO: WhatsApp Business API entegrasyonu
      console.log('[CRM] WhatsApp send not implemented yet');
      errors.push({ channel: 'whatsapp', error: 'WhatsApp henüz aktif değil' });
    }
...
export const cleanupDuplicateMessages = async (conversationId) => {
  try {
    ...
    // Konuşmadaki tüm email mesajlarını al
    ...
    // outlookMessageId'ye göre grupla
    const messageGroups = {};
    snapshot.docs.forEach(doc => {
      ...
      if (key) {
        ...
        messageGroups[key].push({ id: doc.id, ...data, createdAt: data.createdAt?.toDate?.() || new Date() });
      }
    });
    
    // Duplicate'ları bul ve sil
    let deletedCount = 0;
    const batch = writeBatch(db);
    ...
    for (const [key, messages] of Object.entries(messageGroups)) {
      if (messages.length > 1) {
        ...
        console.log(`[CRM] Found ${toDelete.length} duplicates for outlookMessageId: ${key}`);
