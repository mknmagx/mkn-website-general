  ...
  try {
    ...
    if (useTemplate) {
      ...
      if (!options.skipCrmRecord) {
        // CRM mesajı olarak da kaydet - template içeriğini de kaydet
        // WhatsApp servisi renderedText döndürüyor
        const templateContent = result.message?.text || result.renderedText || `[Şablon: ${options.templateName}]`;
        ...
        await addMessage(crmConversationId, {
          ...
          channelMetadata: {
            ...
            renderedContent: templateContent, // Şablon içeriğini de sakla
          },
        });
