...
const getExistingWhatsAppSourceRefs = async () => {
  try {
    ...
    // 1. Önce WhatsApp kanalındaki konuşmaları al
    ...
    // 2. Diğer kanallardaki WhatsApp metadata'sı olan konuşmaları da al
    // (Quote form, Contact form vb. üzerinden WhatsApp iletişimi başlamış olabilir)
    ...
    allConversationsSnapshot.forEach(docSnap => {
      const data = docSnap.data();
      // WhatsApp kanalı zaten yukarıda işlendi, tekrar işleme
      if (data.channel === CHANNEL.WHATSAPP) return;
      
      // Diğer kanallardan WhatsApp iletişimi başlamışsa waId olur
      if (data.channelMetadata?.waId) {
        existingSourceRefs.add(data.channelMetadata.waId);
        console.log(`[WhatsApp Sync] Found waId in ${data.channel} channel: ${data.channelMetadata.waId}`);
      }
      if (data.channelMetadata?.whatsappConversationId) {
        existingSourceRefs.add(data.channelMetadata.whatsappConversationId);
      }
    ...
    console.log(`[WhatsApp Sync] Found ${existingSourceRefs.size} existing WhatsApp source refs (all channels)`);
  ...
  try {
    ...
    for (const waConv of waConversations) {
      try {
        ...
        // Zaten var mı kontrol et (waId, sourceRefId veya whatsappConversationId ile)
...
const updateExistingWhatsAppConversation = async (waConv) => {
  try {
    ...
    // Önce waId ile eşleşen konuşmaları bul
    const allConversationsSnapshot = await getDocs(collection(db, COLLECTIONS.CONVERSATIONS));
    ...
    const crmConvData = crmConvDoc.data();
    
    console.log(`[WhatsApp Sync] Updating existing conversation ${crmConvId} (channel: ${crmConvData.channel}, match: ${matchReason})`);
    ...
    if (!crmConvData.channelMetadata?.whatsappConversationId && whatsappConversationId) {
      ...
      console.log(`[WhatsApp Sync] Linking whatsappConversationId to ${crmConvId}`);
    }
