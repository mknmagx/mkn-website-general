 ...
 * NOT: Bu fonksiyon artık merkezi reminder-service'e yönlendirildi.
 * Eski activity tabanlı reminder'lar için geriye dönük uyumluluk korunmaktadır.
 */
export const getUpcomingReminders = async (userId = null, options = {}) => {
  try {
    ...
    // Önce merkezi reminder koleksiyonundan çek
    const centralReminders = await getCentralReminders(userId, limitCount, daysAhead);
    
    // Eğer merkezi koleksiyonda veri varsa, onu kullan
    if (centralReminders.length > 0) {
      return centralReminders;
    }
    
    // Geriye dönük uyumluluk: Eski activity tabanlı reminder'ları da kontrol et
    ...
    reminders = reminders.filter(r => {
      ...
      if (r.metadata?.isCompleted === true) return false;
...
/**
 * Merkezi reminder koleksiyonundan hatırlatmaları getir (internal helper)
 */
const getCentralReminders = async (userId = null, limitCount = 20, daysAhead = 7) => {
  try {
    const now = new Date();
    ...
    const reminders = snapshot.docs
      ...
      .filter(r => {
        ...
        return r.dueDate <= futureTimestamp;
      })
      .slice(0, limitCount);
    
    return reminders;
  } catch (error) {
    // REMINDERS koleksiyonu yoksa boş dön (geriye dönük uyumluluk)
    console.warn("Central reminders collection may not exist yet:", error.message);
    return [];
  }
};
