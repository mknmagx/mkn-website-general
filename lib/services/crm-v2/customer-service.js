...
let syncServiceLoadFailed = false;

const getSyncService = async () => {
  // Eğer daha önce yüklenemedişse tekrar deneme
  if (syncServiceLoadFailed) {
    return null;
  }
  ...
  if (!syncService) {
    try {
      ...
      console.warn('[CRM] Company sync service could not be loaded:', error.message);
      syncServiceLoadFailed = true;
      return null;
    }
  }
  return syncService;
  ...
  try {
    ...
    if (syncToCompany && !customerData.linkedCompanyId) {
      try {
        ...
        if (sync && sync.onCustomerCreated) {
...
export const updateCustomer = async (customerId, updateData, updatedBy = null, options = {}) => {
  ...
  try {
    ...
    if (syncToCompany) {
      try {
        const sync = await getSyncService();
        if (sync && sync.onCustomerUpdated) {
 ...
 * 
 * Telefon numarası karşılaştırması normalize edilmiş şekilde yapılır:
 * - '+90 536 592 30 35' ve '905365923035' eşleşir
 * - Tüm formatlar aynı müşteriyi bulur
 */
export const findCustomerByContact = async (email = null, phone = null) => {
  try {
    ...
    // Tüm müşterileri çek ve client-side karşılaştırma yap
    // Bu, farklı telefon formatlarının eşleşmesini garantiler
    const allQuery = query(collection(db, COLLECTIONS.CUSTOMERS), limit(1000));
    ...
    for (const doc of allSnapshot.docs) {
      ...
      // Ana telefon eşleşmesi (normalize edilmiş)
      if (normalizedSearchPhone && data.phone) {
        const normalizedCustomerPhone = normalizePhone(data.phone);
        if (normalizedCustomerPhone === normalizedSearchPhone) {
          console.log(`[Customer] Found by normalized phone: ${normalizedSearchPhone}`);
          return { id: doc.id, ...data };
        }
      }
      
      // Alternatif iletişim bilgilerinde ara
      const altContacts = data.alternativeContacts || [];
      
      if (email) {
        const foundEmail = altContacts.find(c => 
          c.type === 'email' && c.value && c.value.toLowerCase() === email.toLowerCase()
        );
        if (foundEmail) {
          return { id: doc.id, ...data };
        }
      ...
      if (normalizedSearchPhone) {
        const foundPhone = altContacts.find(c => 
          c.type === 'phone' && c.value && normalizePhone(c.value) === normalizedSearchPhone
        );
        if (foundPhone) {
          console.log(`[Customer] Found by normalized alt phone: ${normalizedSearchPhone}`);
          return { id: doc.id, ...data };
        }
      }
    }
...
/**
 * Telefon numarasını normalize et - karşılaştırma için
 * Tüm özel karakterleri kaldırır, sadece rakamları bırakır
 * 
 * Örnekler:
 * - '+90 536 592 30 35' → '905365923035'
 * - '0536 592 30 35' → '05365923035'
 * - '905365923035' → '905365923035'
 */
const normalizePhone = (phone) => {
  if (!phone) return '';
  // Tüm rakam olmayan karakterleri kaldır (+ dahil)
