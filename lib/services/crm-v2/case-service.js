...
// Import merkezi reminder servisi
import { 
  createReminder as createCentralReminder,
  completeReminder as completeCentralReminder,
  snoozeReminder as snoozeCentralReminder,
  deleteReminder as deleteCentralReminder,
  REMINDER_ENTITY_TYPE,
} from './reminder-service';

/**
 * Case'e hatırlatma ekle
 * Hem case içine hem de merkezi reminder koleksiyonuna kaydeder
 */
export const addReminderToCase = async (caseId, reminderData, createdBy = null) => {
  try {
    ...
    const newReminder = {
      ...
      createdBy,
      // Merkezi koleksiyondaki ID (sonra eklenecek)
      centralReminderId: null,
    };
    
    // 1. Merkezi koleksiyona kaydet
    let centralReminder = null;
    try {
      centralReminder = await createCentralReminder({
        ...reminderData,
        entityType: REMINDER_ENTITY_TYPE.CASE,
        entityId: caseId,
        entityTitle: caseData.title,
        customerId: caseData.customerId,
        createdBy,
        // Customer bilgilerini de ekle
        customerName: caseData.customerName || null,
      });
      
      // Merkezi ID'yi case reminder'a ekle
      newReminder.centralReminderId = centralReminder.id;
    } catch (centralError) {
      console.warn("Could not create central reminder, continuing with case-only:", centralError);
    }
...
export const updateReminderStatus = async (caseId, reminderId, status, updatedBy = null) => {
  try {
    ...
    const reminders = caseData.reminders || [];
    const targetReminder = reminders.find(rem => rem.id === reminderId);
    ...
    // Merkezi reminder'ı da güncelle
    if (targetReminder?.centralReminderId) {
      try {
        if (status === 'completed') {
          await completeCentralReminder(targetReminder.centralReminderId, updatedBy);
        }
      } catch (centralError) {
        console.warn("Could not update central reminder:", centralError);
      }
    }
 ...
 * Hatırlatmayı ertele
 */
export const snoozeReminder = async (caseId, reminderId, hoursToSnooze, updatedBy = null) => {
  try {
    const caseData = await getCase(caseId);
    if (!caseData) throw new Error('Case not found');
    ...
    newDueDate.setHours(newDueDate.getHours() + hoursToSnooze);
    ...
    const updatedReminders = reminders.map(rem => 
      rem.id === reminderId 
        ? { 
            ...
            snoozedBy: updatedBy,
            snoozeCount: (rem.snoozeCount || 0) + 1,
          }
        : rem
    );
    ...
    if (targetReminder?.centralReminderId) {
      try {
        await snoozeCentralReminder(targetReminder.centralReminderId, hoursToSnooze * 60, updatedBy);
      } catch (centralError) {
        console.warn("Could not snooze central reminder:", centralError);
      }
    }
...
export const deleteReminder = async (caseId, reminderId, deletedBy = null) => {
  try {
    ...
    // Merkezi reminder'ı da sil
    if (targetReminder?.centralReminderId) {
      try {
        await deleteCentralReminder(targetReminder.centralReminderId);
      } catch (centralError) {
        console.warn("Could not delete central reminder:", centralError);
      }
    }
...
// =============================================================================
// AI SUMMARY FUNCTIONS - AI Talep Özeti Yönetimi
// =============================================================================

/**
 * Case'e AI özeti ekle
 * Birden fazla özet tutulabilir (array olarak)
 */
export const addSummaryToCase = async (caseId, summaryData, userId = null, userName = null) => {
  try {
    const caseData = await getCase(caseId);
    if (!caseData) throw new Error('Case not found');
    ...
    const newSummary = {
      ...
      // AI'dan gelen yapılandırılmış özet
      mainRequest: summaryData.mainRequest || '',
      requestedProducts: summaryData.requestedProducts || [], // Talep edilen ürünler
      ...
      // Meta bilgiler
      createdBy: userId,
      createdByName: userName || 'AI',
      createdAt: Timestamp.now(),
      // AI model bilgisi
      aiModel: summaryData.aiModel || null,
      aiProvider: summaryData.aiProvider || null,
      // Hangi mesaj sayısına kadar özet alındığı
      messageCount: summaryData.messageCount || 0,
    };
    
    const aiSummaries = caseData.aiSummaries || [];
    aiSummaries.unshift(newSummary); // En yeni en üstte
    ...
    // Aktivite kaydı
    await logActivity({
      type: 'ai_summary_added',
      caseId,
      customerId: caseData.customerId,
      performedBy: userId,
      metadata: { 
        summaryId: newSummary.id,
        mainRequest: newSummary.mainRequest,
...
/**
 * Case'den AI özet sil
 */
export const deleteSummaryFromCase = async (caseId, summaryId, userId = null) => {
  try {
    const caseData = await getCase(caseId);
    if (!caseData) throw new Error('Case not found');
    
    const aiSummaries = (caseData.aiSummaries || []).filter(s => s.id !== summaryId);
...
/**
 * Case'deki AI özetleri getir
 */
export const getCaseSummaries = async (caseId) => {
  try {
    const caseData = await getCase(caseId);
    if (!caseData) return [];
    
    return caseData.aiSummaries || [];
  } catch (error) {
