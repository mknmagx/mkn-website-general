...
export const reimportEmailById = async (outlookMessageId, createdBy = null) => {
  try {
    ...
    console.log('[CRM Sync] Email reimport debug:', {
      outlookMessageId,
      subject: email.subject,
      bodyType: emailBodyType,
      bodyContentLength: emailBodyContent?.length,
      bodyPreviewLength: fallbackContent?.length,
    ...
    if (!finalContent) {
      return {
        success: false,
        error: 'Email body içeriği boş geldi',
        emptyContent: true,
      };
    }
    
    // 2. CRM'de bu email'i bul - önce messages'ta ara (channelMetadata veya emailMetadata)
    const messagesRef = collection(db, 'crm_messages');
    
    // Önce channelMetadata'da ara (yeni format)
    let q = query(messagesRef, where('channelMetadata.outlookMessageId', '==', outlookMessageId));
    let snapshot = await getDocs(q);
    
    // Bulunamadıysa emailMetadata'da ara (eski format)
    if (snapshot.empty) {
      q = query(messagesRef, where('emailMetadata.outlookMessageId', '==', outlookMessageId));
    ...
    if (!snapshot.empty) {
      ...
      // channelMetadata mı emailMetadata mı kullanıldığını belirle
      const metadataField = messageData.channelMetadata?.outlookMessageId ? 'channelMetadata' : 'emailMetadata';
      
      await updateDoc(doc(db, 'crm_messages', messageId), {
        content: finalContent,
        contentHtml: emailBodyType === 'html' ? emailBodyContent : null,
        [`${metadataField}.bodyType`]: emailBodyType,
        [`${metadataField}.bodyLength`]: finalContent.length,
        [`${metadataField}.reimportedAt`]: new Date().toISOString(),
        [`${metadataField}.reimportedBy`]: createdBy,
        updatedAt: serverTimestamp(),
      });
      
      console.log(`[CRM Sync] Email message updated: ${messageId}, content length: ${finalContent.length}`);
    ...
    if (!convSnapshot.empty) {
      ...
      if (convMsgSnapshot.empty) {
        // İlk mesaj yok - oluştur (channelMetadata kullan - yeni format)
        const messageData = {
        ...
        const firstMsgData = firstMsgDoc.data();
        
        // channelMetadata mı emailMetadata mı kullanıldığını belirle
        const metadataField = firstMsgData.channelMetadata?.outlookMessageId ? 'channelMetadata' : 'emailMetadata';
        ...
        console.log(`[CRM Sync] First email message updated: ${firstMsgId}, content length: ${finalContent.length}`);
...
/**
 * CRM'de mevcut bir WhatsApp mesajının içeriğini whatsapp_messages'dan yeniden al ve güncelle
 * @param {string} wamId - WhatsApp Message ID
 * @param {string} createdBy - İşlemi yapan kullanıcı
 * @returns {Object} Güncelleme sonucu
 */
export const reimportWhatsAppMessageById = async (wamId, createdBy = null) => {
  try {
    // 1. whatsapp_messages'dan mesajı bul
    const waMessagesRef = collection(db, 'whatsapp_messages');
    const waQuery = query(waMessagesRef, where('wamId', '==', wamId), limit(1));
    const waSnapshot = await getDocs(waQuery);
    
    if (waSnapshot.empty) {
      return {
        success: false,
        error: 'WhatsApp mesajı bulunamadı',
        notFound: true,
      };
    }
    
    const waMessage = waSnapshot.docs[0].data();
    const realContent = waMessage.text || waMessage.content?.text || '';
    
    if (!realContent) {
      return {
        success: false,
        error: 'WhatsApp mesajında içerik bulunamadı',
        emptyContent: true,
      };
    }
    
    // 2. CRM'de bu mesajı bul
    const messagesRef = collection(db, 'crm_messages');
    const crmQuery = query(messagesRef, where('channelMetadata.wamId', '==', wamId));
    const crmSnapshot = await getDocs(crmQuery);
    
    if (crmSnapshot.empty) {
      return {
        success: false,
        error: 'Mesaj CRM\'de bulunamadı',
        notFound: true,
      };
    }
    
    // Mesajı güncelle
    const messageDoc = crmSnapshot.docs[0];
    const messageId = messageDoc.id;
    const messageData = messageDoc.data();
    
    await updateDoc(doc(db, 'crm_messages', messageId), {
      content: realContent,
      'channelMetadata.renderedContent': realContent,
      'channelMetadata.reimportedAt': new Date().toISOString(),
      'channelMetadata.reimportedBy': createdBy,
      updatedAt: serverTimestamp(),
    });
    ...
    return {
      ...
      oldContent: messageData.content?.substring(0, 50),
      newContent: realContent.substring(0, 50),
    };
    
  } catch (error) {
    console.error('[CRM Sync] Error reimporting WhatsApp message:', error);
    return {
      success: false,
      error: error.message,
    };
...
/**
 * Bir conversation'daki tüm mesajları getir (yönetim için)
 * @param {string} conversationId - Conversation ID
 * @returns {Array} Mesajlar
 */
export const getConversationMessagesForManagement = async (conversationId) => {
  try {
    const messagesRef = collection(db, 'crm_messages');
    const q = query(
      messagesRef,
    ...
    console.error('[CRM Sync] Error getting messages for management:', error);
    return [];
  }
};
