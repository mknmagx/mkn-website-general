...
export const createOrder = async (orderData, userId) => {
  try {
    ...
    const newOrder = {
      ...
      production:
        orderData.type === ORDER_TYPE.PRODUCTION
          ? {
              batchNumber: orderData.production?.batchNumber || "",
              lotNumber: orderData.production?.lotNumber || "",
              formulaId:
                orderData.formulaId || orderData.production?.formulaId || null,
              formulaName:
                orderData.formulaName ||
                orderData.production?.formulaName ||
                null,
...
export const createOrderFromCase = async (caseData, proformaData, userId) => {
  try {
    ...
    if (!proformaData && caseData.quotes?.length > 0) {
      const acceptedQuote =
        caseData.quotes.find((q) => q.status === "accepted") ||
        caseData.quotes[0];
      if (acceptedQuote) {
        ...
        subtotal = total / 1.2; // KDV çıkar (varsayılan %20)
      }
    }
...
export const unlinkDeliveryFromOrder = async (orderId, deliveryId, userId) => {
  try {
    ...
    const deliveryIds = (order.deliveryIds || []).filter(
      (id) => id !== deliveryId,
    );
 ...
 * Firebase does not accept undefined values in updateDoc
 */
 ...
 */
export const updateProductionDetails = async (
  orderId,
  productionData,
  userId,
) => {
  try {
    ...
    // Merge with existing production data
    ...
    // Clean undefined values - Firebase doesn't accept undefined
    const production = removeUndefinedValues(mergedProduction);
...
export const deleteChecklistItem = async (orderId, checklistItemId, userId) => {
  try {
    ...
    if (!item?.isCustom) {
      return {
        success: false,
        error: "Sadece özel eklenen görevler silinebilir",
      };
    }

    const checklist = (order.checklist || []).filter(
      (i) => i.id !== checklistItemId,
    );
...
export const updateFormulaInfo = async (orderId, formulaData, userId) => {
  try {
    ...
    if (!order || order.type !== ORDER_TYPE.PRODUCTION) {
      return {
        success: false,
        error: "Geçerli bir üretim siparişi bulunamadı",
      };
    }

    const production = removeUndefinedValues({
      ...(order.production || {}),
      formulaId: formulaData.formulaId || null,
      formulaName: formulaData.formulaName || null,
      formulaCode: formulaData.formulaCode || null,
  ...
  try {
    ...
    const production = removeUndefinedValues({
      ...
      packaging: {
        ...
        ...packagingData,
        approvedAt: packagingData.approved
          ? Timestamp.now()
          : order.production?.packaging?.approvedAt || null,
      },
    });
  ...
  try {
    ...
    const production = removeUndefinedValues({
      ...
      label: {
        ...
        ...labelData,
        approvedAt: labelData.approved
          ? Timestamp.now()
          : order.production?.label?.approvedAt || null,
      },
    });
  ...
  try {
    ...
    const production = removeUndefinedValues({
      ...
      box: {
        ...
        ...boxData,
        approvedAt: boxData.approved
          ? Timestamp.now()
          : order.production?.box?.approvedAt || null,
      },
    });
 ...
 */
export const updateFinalPackagingInfo = async (
  orderId,
  packagingInfoData,
  userId,
) => {
 ...
 * Kontrat bağla
 */
export const linkContractToOrder = async (
  orderId,
  contractId,
  contractNumber,
  userId,
...
export const linkProformaToOrder = async (orderId, proformaData, userId) => {
  try {
    ...
    if (proformaData.customerInfo) {
      updateData.customer = {
        companyName:
          proformaData.customerInfo.companyName ||
          order.customer?.companyName ||
          "",
        contactName:
          proformaData.customerInfo.contactPerson ||
 ...
 */
export const createOrderFromProforma = async (
  proformaData,
  orderType,
  userId,
) => {
