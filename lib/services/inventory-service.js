...
export const inventoryItemService = {
  ...
  async update(id, data, user) {
    try {
      ...
      // Update warehouseId and supplierId in related transactions if changed
      const warehouseIdChanged = data.warehouseId !== undefined && data.warehouseId !== currentItem.warehouseId;
      const supplierIdChanged = data.supplierId !== undefined && data.supplierId !== currentItem.supplierId;
      const supplierNameChanged = data.supplierName !== undefined && data.supplierName !== currentItem.supplierName;

      if (warehouseIdChanged || supplierIdChanged || supplierNameChanged) {
        await this.updateRelatedTransactionsWarehouseAndSupplier(id, {
          warehouseId: data.warehouseId !== undefined ? updatedItem.warehouseId : undefined,
          supplierId: data.supplierId !== undefined ? updatedItem.supplierId : undefined,
          supplierName: data.supplierName !== undefined ? updatedItem.supplierName : undefined,
        });
      }
  ...
  async updateRelatedTransactionsWarehouseAndSupplier(itemId, { warehouseId, supplierId, supplierName }) {
    try {
      // Get all transactions for this item
      const transactionsQuery = query(
        collection(db, INVENTORY_COLLECTIONS.TRANSACTIONS),
      ...
      snapshot.docs.forEach((docSnap) => {
        ...
        if (warehouseId !== undefined) {
          updateObj.warehouseId = warehouseId;
        }
        
        // Update supplierId if provided
        if (supplierId !== undefined) {
          updateObj.supplierId = supplierId;
        }

        // Update supplierName if provided
        if (supplierName !== undefined) {
          updateObj.supplierName = supplierName;
        }
      ...
      // Don't throw - this is a secondary operation
      console.error("Error updating related transactions warehouse/supplier:", error);
    }
  },
