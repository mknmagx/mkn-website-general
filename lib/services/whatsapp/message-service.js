...
export async function sendImageMessage(conversationId, to, imageUrl, caption = null, filename = null) {
  try {
    ...
    const response = await sendMediaMessage(to, 'image', imageUrl, caption, filename);
...
export async function sendGenericMediaMessage(conversationId, to, type, mediaUrl, caption = null, filename = null) {
  try {
    ...
    const response = await sendMediaMessage(to, type, mediaUrl, caption, filename);
...
export async function processIncomingMessage(webhookMessage, contact = null, phoneNumberId = null) {
  try {
    ...
    // Update service window (24-hour messaging window) in WhatsApp conversation
    await updateServiceWindow(conversation.id);

    // ✅ Also update CRM conversation's service window if linked
    try {
      const { refreshCrmServiceWindow } = await import('../crm-v2/whatsapp-sync-service');
      await refreshCrmServiceWindow(from, conversation.id);
    } catch (crmSyncError) {
      // Don't fail message processing if CRM sync fails
      console.warn('[WhatsApp] CRM service window sync failed:', crmSyncError.message);
    }
