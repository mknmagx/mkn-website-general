 ...
 * @param {Array} linkedCompanyIds - Optional array of company IDs to link
 * @returns {Promise<string>} Document ID
 */
export async function savePricingCalculation(data, linkedCompanyIds = []) {
  const docRef = await addDoc(collection(db, COLLECTION), {
    ...data,
    linkedCompanies: linkedCompanyIds || [], // Array of company IDs
...
export function calculateIngredientPercentage(amount, unit, totalAmount) {
  // "adet" birimi yüzde hesaplamasına dahil edilmez
...
export function calculateTotalProductVolume(ingredients) {
  return ingredients.reduce((sum, ing) => {
    // "adet" birimi toplam hacim hesaplamasına dahil edilmez
    if (ing.unit === "adet") return sum;
...
export function calculatePricing(formData) {
  ...
  // Formülden gelen kutu içindeki adet (örn: 60 kapsül/kutu)
  // Bu değer varsa, hammadde maliyetleri bu çarpanla hesaplanır
  const boxQuantityMultiplier = parseFloat(formData.sourceFormulaProductionQuantity) || 1;

  // Calculate total product volume (for one unit)
  const totalProductVolume = calculateTotalProductVolume(formData.ingredients);

  // Calculate ingredient costs for ONE unit (considering box quantity multiplier)
  // Formüldeki miktarlar 1 birim ürün içindir, kutu içindeki adet ile çarpıyoruz
  let ingredientsCostPerUnit = 0;
  const ingredientDetails = formData.ingredients.map((ing) => {
    const amount = parseFloat(ing.amount) || 0;
    const price = parseFloat(ing.price) || 0;

    // 1 birim ürün için hammadde maliyeti
    const costPerSingleUnit = calculateIngredientCost(amount, ing.unit, price);
    
    // Kutu başına maliyet (1 birim * kutu içindeki adet)
    const costPerBox = costPerSingleUnit * boxQuantityMultiplier;
    ...
    ingredientsCostPerUnit += costPerBox;

    return {
      name: ing.name,
      amount,
      unit: ing.unit,
      percentage: percentage !== null ? percentage.toFixed(2) : null, // "adet" için null
      pricePerKg: price.toFixed(2),
      costPerSingleUnit: costPerSingleUnit.toFixed(4), // 1 birim için maliyet
      costPerUnit: costPerBox.toFixed(2), // Kutu başına maliyet (boxQuantityMultiplier ile çarpılmış)
      costTotal: (costPerBox * quantity).toFixed(2),
    };
  });
  ...
  return {
    ...
    ingredientDetails,
    quantity,
    boxQuantityMultiplier, // Kutu içindeki adet (formülden gelen)
