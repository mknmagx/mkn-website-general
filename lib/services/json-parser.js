...
export function cleanJsonString(jsonString) {
  ...
  try {
    ...
    logger.error('JSON cleaning error:', { error: error.message });
  ...
  try {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.info('Cleaned JSON (first 300 chars):', cleanedJson.substring(0, 300));
      }
      ...
      logger.warn('Standard JSON parsing failed, trying manual parsing...', { error: jsonError.message });
    ...
    if (error instanceof SyntaxError) {
      ...
      if (match) {
        ...
        logger.error('JSON error context around position:', { position, context });
      }
    }
