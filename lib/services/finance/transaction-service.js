...
import { updateAccountBalance, getAccount, exchangeWithinAccount } from "./account-service";
...
export const createTransaction = async (transactionData, userId) => {
  try {
    ...
    // Transfer işlemi için iki ayrı kayıt oluştur (kaynak ve hedef hesap için)
    if (newTransaction.type === TRANSACTION_TYPE.TRANSFER && newTransaction.toAccountId) {
      const fromCurrency = transactionData.fromCurrency || newTransaction.currency;
      const toCurrency = transactionData.toCurrency || newTransaction.currency;
      const toAmount = transactionData.toAmount || newTransaction.amount;
      
      // Transfer bağlantı ID'si oluştur
      const transferGroupId = `TRF-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      
      // Kaynak hesap için TRANSFER_OUT kaydı
      const outgoingTransaction = {
        ...newTransaction,
        type: TRANSACTION_TYPE.TRANSFER,
        transferDirection: 'OUT', // Çıkış
        transferGroupId, // İki kaydı bağlamak için
        currency: fromCurrency,
        description: newTransaction.description || `${newTransaction.toAccountName || 'Hedef hesap'}'a transfer`,
      };
      
      const outDocRef = await addDoc(collection(db, COLLECTION), outgoingTransaction);
      
      // Hedef hesap için TRANSFER_IN kaydı
      const incomingTransactionNumber = await generateTransactionNumber(TRANSACTION_TYPE.TRANSFER);
      const incomingTransaction = {
        ...newTransaction,
        transactionNumber: incomingTransactionNumber,
        type: TRANSACTION_TYPE.TRANSFER,
        transferDirection: 'IN', // Giriş
        transferGroupId, // İki kaydı bağlamak için
        accountId: newTransaction.toAccountId, // Hedef hesap artık ana hesap
        accountName: newTransaction.toAccountName,
        toAccountId: newTransaction.accountId, // Kaynak hesap referansı
        toAccountName: newTransaction.accountName,
        amount: toAmount,
        currency: toCurrency,
        description: newTransaction.description || `${newTransaction.accountName || 'Kaynak hesap'}'dan transfer`,
      };
      
      const inDocRef = await addDoc(collection(db, COLLECTION), incomingTransaction);
    ...
    if (newTransaction.status === TRANSACTION_STATUS.COMPLETED) {
      // Normal gelir/gider - işlemin para biriminde bakiyeyi güncelle
      await updateAccountBalance(
        newTransaction.accountId, 
        newTransaction.amount, 
        newTransaction.type, 
...
/**
 * İşlem güncelle (multi-currency destekli)
 */
export const updateTransaction = async (transactionId, updateData, userId) => {
  try {
    ...
    const shouldUpdateBalance = 
      ...
      updateData.currency !== undefined && updateData.currency !== oldTx.currency ||
    ...
    if (shouldUpdateBalance && oldTx.status === TRANSACTION_STATUS.COMPLETED) {
      ...
      await updateAccountBalance(oldTx.accountId, oldTx.amount, reverseType, userId, oldTx.currency);
    }
    ...
    if (shouldUpdateBalance && newStatus === TRANSACTION_STATUS.COMPLETED) {
      ...
      const newCurrency = updateData.currency || oldTx.currency;
      
      await updateAccountBalance(newAccountId, newAmount, newType, userId, newCurrency);
    }
...
/**
 * İşlem sil (multi-currency destekli)
 * - Hesap bakiyelerini geri alır
 * - Transfer işlemlerinde her iki hesabı da düzeltir
 * - Exchange işlemlerinde döviz bakiyelerini geri alır
 */
export const deleteTransaction = async (transactionId, userId) => {
  try {
    ...
    if (tx.status === TRANSACTION_STATUS.COMPLETED) {
      // 1. Exchange işlemleri
      if (tx.type === TRANSACTION_TYPE.EXCHANGE && tx.exchange) {
        const { fromCurrency, toCurrency, fromAmount, toAmount } = tx.exchange;
        
        // Kaynak para birimini geri ekle (çıkış yapılmıştı, geri iade)
        await updateAccountBalance(
          tx.accountId, 
          fromAmount, 
          TRANSACTION_TYPE.INCOME, 
        ...
        // Hedef para birimini geri çıkar (giriş yapılmıştı, geri al)
        await updateAccountBalance(
          tx.accountId, 
          toAmount, 
          TRANSACTION_TYPE.EXPENSE, 
      ...
      // 2. Transfer işlemleri
      else if (tx.type === TRANSACTION_TYPE.TRANSFER && tx.toAccountId) {
        // Kaynak hesaba geri ekle
        await updateAccountBalance(
          tx.accountId, 
          tx.amount, 
          TRANSACTION_TYPE.INCOME, 
      ...
      // 3. Normal gelir/gider işlemleri
      else {
        const reverseType = tx.type === TRANSACTION_TYPE.INCOME 
          ? TRANSACTION_TYPE.EXPENSE 
          : TRANSACTION_TYPE.INCOME;
        
        await updateAccountBalance(tx.accountId, tx.amount, reverseType, userId, tx.currency);
      }
    }
    ...
    return {
      success: true,
      message: "İşlem başarıyla silindi ve bakiyeler güncellendi",
    };
  } catch (error) {
...
// =============================================================================
// DÖVİZ EXCHANGE İŞLEMLERİ
// =============================================================================

/**
 * Döviz çevirme işlemi oluştur (hesap içi exchange)
 * @param {Object} exchangeData - Exchange detayları
 * @param {string} exchangeData.accountId - Hesap ID
 * @param {string} exchangeData.fromCurrency - Kaynak döviz
 * @param {string} exchangeData.toCurrency - Hedef döviz
 * @param {number} exchangeData.fromAmount - Kaynak miktar
 * @param {number} exchangeData.toAmount - Hedef miktar
 * @param {number} exchangeData.exchangeRate - Kur
 * @param {string} userId - Kullanıcı ID
 */
export const createExchangeTransaction = async (exchangeData, userId) => {
  try {
    const {
      accountId,
      fromCurrency,
    ...
    const exchangeResult = await exchangeWithinAccount(
      accountId,
      fromCurrency,
      toCurrency,
      fromAmount,
    ...
    // Exchange işlem numarası oluştur
    const year = new Date().getFullYear();
    const month = String(new Date().getMonth() + 1).padStart(2, '0');
    
    const q = query(
      collection(db, COLLECTION),
      where("type", "==", TRANSACTION_TYPE.EXCHANGE),
      where("transactionNumber", ">=", `EXC-${year}${month}-`),
      where("transactionNumber", "<=", `EXC-${year}${month}-\uf8ff`),
      orderBy("transactionNumber", "desc"),
      limit(1)
    );
    ...
    const transactionNumber = `EXC-${year}${month}-${String(nextNumber).padStart(4, "0")}`;
    ...
    const newTransaction = {
      ...
      // Gösterim için ana tutar (kaynak miktar)
      amount: fromAmount,
      currency: fromCurrency,
      
      // Detaylar
      description: description || `${fromAmount} ${fromCurrency} → ${toAmount} ${toCurrency} (Kur: ${exchangeRate})`,
    ...
    return {
      ...
      id: docRef.id,
      transactionNumber,
      newBalances: exchangeResult.newBalances,
      message: "Döviz çevirme işlemi başarıyla kaydedildi",
    };
  } catch (error) {
...
/**
 * Çoklu döviz transferi (farklı döviz cinslerinde hesaplar arası)
 */
export const createCrossTransferTransaction = async (transferData, userId) => {
  try {
    const {
      fromAccountId,
      toAccountId,
    ...
    // Kaynak hesaptan düş
    const fromResult = await updateAccountBalance(
      fromAccountId, 
      fromAmount, 
      TRANSACTION_TYPE.EXPENSE, 
      userId,
    ...
    // Hedef hesaba ekle
    const toResult = await updateAccountBalance(
      toAccountId, 
      toAmount, 
      TRANSACTION_TYPE.INCOME, 
      userId,
    ...
    // Transfer işlem numarası oluştur
    const year = new Date().getFullYear();
    const month = String(new Date().getMonth() + 1).padStart(2, '0');
    
    const q = query(
      collection(db, COLLECTION),
      where("type", "==", TRANSACTION_TYPE.TRANSFER),
      where("transactionNumber", ">=", `TRF-${year}${month}-`),
      where("transactionNumber", "<=", `TRF-${year}${month}-\uf8ff`),
      orderBy("transactionNumber", "desc"),
      limit(1)
    );
    ...
    const transactionNumber = `TRF-${year}${month}-${String(nextNumber).padStart(4, "0")}`;
    const incomingTransactionNumber = `TRF-${year}${month}-${String(nextNumber + 1).padStart(4, "0")}`;
    ...
    const outgoingTransaction = {
      ...
      fromAmount,
      toAmount,
      exchangeRate: fromCurrency !== toCurrency ? exchangeRate : null,
      ...
      // Detaylar
      description: description || (fromCurrency !== toCurrency 
        ? `${toAmount} ${toCurrency} olarak transfer gönderildi`
        : `Transfer gönderildi`),
    ...
    const incomingTransaction = {
      ...
      // Transfer detayları - ters çevrilmiş
      accountId: toAccountId, // Hedef hesap artık ana hesap
      toAccountId: fromAccountId, // Kaynak hesap referansı
      fromCurrency: toCurrency,
      toCurrency: fromCurrency,
      fromAmount: toAmount,
      toAmount: fromAmount,
      exchangeRate: fromCurrency !== toCurrency ? (1 / exchangeRate) : null,
      ...
      // Detaylar
      description: description || (fromCurrency !== toCurrency 
        ? `${fromAmount} ${fromCurrency} olarak transfer alındı`
        : `Transfer alındı`),
    ...
    console.error("Error creating cross transfer transaction:", error);
    return {
      success: false,
      error: error.message,
    };
