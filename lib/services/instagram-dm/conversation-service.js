...
/**
 * Konuşmanın tüm mesajlarını siler (conversation kaydını korur)
 * @param {string} conversationId - Conversation ID
 * @returns {Promise<void>}
 */
export async function clearMessages(conversationId) {
  try {
    ...
    // Konuşmaya ait tüm mesajları bul ve sil
    const messagesSnapshot = await adminDb.collection(COLLECTIONS.MESSAGES)
    ...
    // Konuşmayı güncelle - mesaj önizlemesini temizle
    ...
    console.log(`✅ Cleared ${messagesSnapshot.size} messages from conversation ${conversationId}`);
  } catch (error) {
...
/**
 * Konuşmayı ve tüm mesajlarını kalıcı olarak siler
 * @param {string} conversationId - Conversation ID
 * @returns {Promise<void>}
 */
