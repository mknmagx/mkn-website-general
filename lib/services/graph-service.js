...
export async function getEmailById(messageId, userId = EMAIL_SENDER) {
  try {
    const response = await graphApiCall(`/users/${encodeURIComponent(userId)}/messages/${encodeURIComponent(messageId)}`);
    return response;
  } catch (error) {
...
export async function searchEmails(searchQuery, userId = EMAIL_SENDER, options = {}) {
  try {
    ...
    const response = await graphApiCall(
      `/users/${encodeURIComponent(userId)}/messages?$search="${searchQuery}"&$top=${top}&$orderby=receivedDateTime desc`
    );
