...
export class ShopifyIntegrationService {
   ...
   * @param {string} dataSource - Veri kaynağı: 'webhook' | 'sync' | 'manual'
   * @param {Object} webhookInfo - Webhook bilgileri (eğer webhook'tan geliyorsa)
   */
  async saveOrder(
    integrationId,
    companyId,
    ...
    try {
      ...
      // 🔒 RACE CONDITION FIX: Transaction dışı query kaldırıldı
      // Conflict detection artık transaction içinde yapılacak

      // 1. RAW DATA KAYDETME - API'den gelen tam response
      // Raw data kaydı ayrı try-catch ile korunuyor
      try {
        await this.saveRawOrderData(integrationId, companyId, shopifyOrder, dataSource, webhookInfo);
      } catch (rawDataError) {
        logger.error(`Failed to save raw order data for ${shopifyOrder.id}, continuing with processed data`, {
          error: rawDataError.message,
          orderId: shopifyOrder.id,
          source: dataSource
        });
        // Raw data hatası critical değil, processed data'ya devam et
      }
  ...
  async saveRawOrderData(integrationId, companyId, shopifyOrder, dataSource = 'sync', webhookInfo = null) {
    try {
      ...
      if (existingRawOrders.empty) {
        ...
        const existingRawData = existingRawOrders.docs[0].data();
        
        // 🔄 Raw data her zaman tam API response ile güncellenir
        // Ancak metadata alanlarını (webhook history) biriktir
        const updatePayload = {
          rawResponse: shopifyOrder, // Her zaman en son API response
          dataSource,
          syncedAt: new Date(),
          lastUpdatedAt: new Date(),
        };
        
        // Webhook bilgilerini koru ve güncelle
        if (webhookInfo) {
          updatePayload.lastWebhookTopic = webhookInfo.topic;
          updatePayload.lastWebhookId = webhookInfo.webhookId;
          updatePayload.lastWebhookReceivedAt = webhookInfo.receivedAt;
          updatePayload.webhookShopDomain = webhookInfo.shopDomain;
          
          // İlk webhook bilgisini sakla
          if (!existingRawData.firstWebhookReceivedAt) {
            updatePayload.firstWebhookReceivedAt = webhookInfo.receivedAt;
            updatePayload.firstWebhookTopic = webhookInfo.topic;
          }
        } else if (dataSource === 'sync') {
          // Sync ise sync bilgilerini güncelle
          updatePayload.lastSyncAt = new Date();
          if (!existingRawData.firstSyncAt) {
            updatePayload.firstSyncAt = new Date();
          }
        }
        
        await existingRawRef.update(updatePayload);
      }
    } catch (error) {
  ...
  async saveProcessedOrderData(
    ...
    try {
      ...
      // 🔒 PERFORMANCE FIX: Customer enhancement sadece gerekirse ve credentials varsa
      // Webhook'lar genelde full customer data içerir, API call'a gerek olmayabilir
      ...
      if (needsEnhancement) {
        try {
          ...
          logger.warn(`Customer data enhancement failed for order ${shopifyOrder.id}`, {
            error: enhanceError.message,
            customerId: shopifyOrder.customer?.id
          });
          // Enhancement başarısız olsa bile order'ı kaydetmeye devam et
        }
      } else {
        logger.debug(`Skipping customer enhancement for order ${shopifyOrder.id}`, {
          hasCredentials: !!credentials,
          hasShopName: !!shopName,
          hasCustomer: !!shopifyOrder.customer?.id,
          customerState: shopifyOrder.customer?.state
        });
      }
      ...
      // 🔒 FIRESTORE TRANSACTION - Atomic operation for race condition prevention
      let action = 'created';
      ...
      try {
        const result = await adminFirestore.runTransaction(async (transaction) => {
          ...
          if (existingOrders.empty) {
            ...
            // Mevcut kayıt var - güncelleme yap
            const existingOrderRef = existingOrders.docs[0].ref;
            const existingData = existingOrders.docs[0].data();
            
            // 🔍 CONFLICT DETECTION - Webhook vs Sync çakışması önleme
            const orderUpdatedAt = new Date(shopifyOrder.updated_at);
            const existingUpdatedAt = existingData.updatedAt ? new Date(existingData.updatedAt) : null;
            
            // Webhook source - eski data ile üstüne yazma
            if (dataSource === 'webhook') {
              // Mevcut data daha yeni ise webhook'u ignore et
              if (existingUpdatedAt && existingUpdatedAt > orderUpdatedAt) {
              ...
              // Son sync daha yeni ise uyar ama yine de güncelle (webhook genelde daha güncel)
              if (existingData.lastSyncAt) {
                ...
                if (lastSyncAt > orderUpdatedAt) {
                  logger.warn(`Webhook updating order with older timestamp than last sync`, {
                    orderId: shopifyOrder.id,
            ...
            // Sync source - webhook'tan gelen değerli veriyi koruma
            if (dataSource === 'sync' && existingData.lastWebhookAt) {
              const lastWebhookAt = existingData.lastWebhookAt.toDate ? existingData.lastWebhookAt.toDate() : new Date(existingData.lastWebhookAt);
              
              // Webhook'tan gelmiş ve çok yakın zamanda güncellenmişse dikkatli ol
              const timeSinceWebhook = Date.now() - lastWebhookAt.getTime();
              if (timeSinceWebhook < 5 * 60 * 1000) { // 5 dakika
                logger.info(`Sync updating order recently updated by webhook`, {
                  orderId: shopifyOrder.id,
                  lastWebhookAt: lastWebhookAt.toISOString(),
                  minutesSinceWebhook: Math.floor(timeSinceWebhook / 60000),
                  source: dataSource,
                  note: 'Field-level merge will preserve webhook data'
                });
              }
            }
            
            // 🛡️ FIELD-LEVEL MERGE STRATEGY - Veri kaybını önle
            // Sadece yeni, anlamlı değerleri güncelle; null/undefined ile mevcut veriyi silme
            const updateData = {};
            
            for (const [key, value] of Object.entries(orderData)) {
              // 🔍 Değerin anlamlı olup olmadığını kontrol et
              const isValueMeaningful = this.isValueMeaningful(value);
              const isExistingMeaningful = this.isValueMeaningful(existingData[key]);
              
              // Yeni değer anlamlı ise ekle
              if (isValueMeaningful) {
                updateData[key] = value;
              } 
              // Yeni değer boş AMA mevcut data'da da boş ise güncelle (tutarlılık için)
              else if (!isExistingMeaningful) {
                updateData[key] = value;
              }
              // Aksi halde: Mevcut değerli veriyi KORU, değiştirme!
            }
            
            // Tracking alanlarını her zaman güncelle
            updateData.updatedAt = new Date();
            updateData.syncedAt = new Date();
            
            if (dataSource === 'webhook') {
              updateData.lastWebhookAt = new Date();
              if (webhookInfo?.topic) updateData.lastWebhookTopic = webhookInfo.topic;
              if (webhookInfo?.webhookId) updateData.lastWebhookId = webhookInfo.webhookId;
            } else if (dataSource === 'sync') {
              updateData.lastSyncAt = new Date();
            }
            
            // Temizlenmiş update data
            const cleanUpdateData = this.removeUndefinedValues(updateData);
            
            logger.info(`Updating order with field-level merge strategy`, {
              orderId: shopifyOrder.id,
              source: dataSource,
              fieldsUpdated: Object.keys(cleanUpdateData).length,
              preservedFields: Object.keys(existingData).filter(k => !cleanUpdateData.hasOwnProperty(k)).length
            });
            
            transaction.update(existingOrderRef, cleanUpdateData);
      ...
      // Customer'ı ayrı kaydet (transaction dışında, ancak korumalı)
      if (shopifyOrder.customer) {
        try {
          ...
          logger.error(`Failed to save customer data for order ${shopifyOrder.id}`, {
            error: customerError.message,
            customerId: shopifyOrder.customer?.id,
            orderId: shopifyOrder.id
          });
          // Customer kaydı başarısız olsa bile order başarıyla kaydedildi
        }
      }
  ...
  /**
   * Webhook için mevcut order kaydını kontrol et
   * @deprecated Bu fonksiyon artık saveProcessedOrderData'da transaction içinde yapılıyor
   * @param {string} integrationId - Integration ID
  ...
  async saveCustomerData(integrationId, companyId, customerData) {
    try {
      ...
      if (existingCustomers.empty) {
        ...
        // 🛡️ FIELD-LEVEL MERGE STRATEGY - Customer verilerini koru
        const updateData = {};
        let hasActualChanges = false;
        
        for (const [key, value] of Object.entries(cleanCustomerData)) {
          // syncedAt, createdAt gibi tracking alanlarını atla
          if (key === 'syncedAt' || key === 'createdAt') continue;
          ...
          // Yeni değer anlamlı ise ekle
          if (isValueMeaningful) {
            // Değer gerçekten değişmiş mi?
            if (JSON.stringify(existingData[key]) !== JSON.stringify(value)) {
              updateData[key] = value;
              hasActualChanges = true;
            }
          } 
          // Yeni değer boş AMA mevcut data'da da boş ise güncelle
          else if (!isExistingMeaningful) {
            if (JSON.stringify(existingData[key]) !== JSON.stringify(value)) {
              updateData[key] = value;
              hasActualChanges = true;
          ...
          // Aksi halde: Mevcut değerli veriyi KORU
        }
        ...
        if (hasActualChanges) {
          ...
          logger.info(`Updating customer with field-level merge`, {
            customerId: customerData.id,
            fieldsUpdated: Object.keys(cleanUpdateData).length
          });

          await existingCustomerRef.update(cleanUpdateData);
          return "updated";
        } else {
  ...
  /**
   * Değerin anlamlı olup olmadığını kontrol et
   * Boş array, boş object, null, undefined, empty string anlamlı değildir
   */
  isValueMeaningful(value) {
    ...
    // Empty object (ancak Date, Firestore Timestamp gibi özel objeler hariç)
    if (typeof value === 'object' && value.constructor === Object) {
      const keys = Object.keys(value);
      if (keys.length === 0) return false;
      
      // Tüm değerleri null/undefined olan object'ler de anlamsız
      const hasAnyMeaningfulValue = keys.some(key => this.isValueMeaningful(value[key]));
      if (!hasAnyMeaningfulValue) return false;
    }
    
    // Diğer tüm durumlar anlamlı
    return true;
  }
  ...
  async saveReturn(integrationId, companyId, refundedOrder) {
    try {
      ...
      if (existingReturns.empty) {
        ...
        // 🛡️ FIELD-LEVEL MERGE STRATEGY - Return verilerini koru
        const existingReturnRef = existingReturns.docs[0].ref;
        const existingData = existingReturns.docs[0].data();
        
        const updateData = {};
        
        for (const [key, value] of Object.entries(returnData)) {
          // syncedAt, createdAt gibi tracking alanlarını atla
          if (key === 'syncedAt' || key === 'refundedAt') continue;
        ...
        logger.info(`Updating return with field-level merge`, {
          returnId: refundedOrder.id,
          fieldsUpdated: Object.keys(cleanUpdateData).length
        });
        
        await existingReturnRef.update(cleanUpdateData);
      }
    } catch (error) {
  ...
  async removeWebhooks(integrationId) {
    try {
      ...
      if (!webhooks || webhooks.length === 0) {
        // Webhook yoksa sadece database'deki webhook kayıtlarını temizle
        const cleanupResult = await this.cleanupWebhookRecords(integrationId);
        return {
          success: true,
          message: "Silinecek aktif webhook bulunamadı, webhook kayıtları temizlendi",
          removed: [],
      ...
      // Webhook'lar silindikten sonra database'deki webhook kayıtlarını temizle
      ...
      if (removedWebhooks.length > 0) {
        try {
          ...
          // Cleanup hatası webhook kaldırma başarısını etkilemez
        }
      }
  ...
  async createWebhook(credentials, shopName, webhookData) {
    try {
      ...
      // Webhook secret'ı ensure et (webhookData'dan veya credentials'dan)
      ...
      // Webhook secret varsa Shopify'a gönder (HMAC imzalama için)
      if (webhookSecret) {
        webhookPayload.webhook.fields = [];  // Tüm field'ları gönder
        // NOT: Shopify API'de webhook secret doğrudan gönderilemez
        // Secret, webhook receiver'da HMAC doğrulaması için kullanılır
        // Shopify otomatik olarak kendi secret'ını kullanır ve HMAC oluşturur
      }
  ...
  /**
   * Webhook kayıtlarını güvenli şekilde temizle
   * Sadece webhook koleksiyonundaki kayıtları siler, sipariş/müşteri verilerine dokunmaz
   * @param {string} integrationId - Integration ID
  ...
  async cleanupWebhookRecords(integrationId) {
    try {
      ...
      // 1. Shopify Webhooks kayıtlarını temizle (deleted status olanlar)
      try {
        const webhooksSnapshot = await adminFirestore
          .collection(SHOPIFY_WEBHOOKS_COLLECTION)
          .where("integrationId", "==", integrationId)
   ...
   * @param {string} integrationId - Integration ID
   * @param {number} days - Son kaç günlük veri
  ...
  async getDataSourceStats(integrationId, days = 7) {
    try {
      ...
      // Orders collection'dan istatistikleri çek
      const ordersSnapshot = await adminFirestore
        .collection(SHOPIFY_ORDERS_COLLECTION)
        .where("integrationId", "==", integrationId)
      ...
      ordersSnapshot.docs.forEach(doc => {
        ...
        if (data.lastWebhookAt && data.lastSyncAt) {
          ...
          // Eğer webhook ve sync 5 dakika içinde olmuşsa conflict sayılır
  ...
  async getDataConflictReport(integrationId, days = 7) {
    try {
      ...
      // Conflict potansiyeli olan siparişleri bul
      const ordersSnapshot = await adminFirestore
        .collection(SHOPIFY_ORDERS_COLLECTION)
        .where("integrationId", "==", integrationId)
        .where("syncedAt", ">=", startDate)
      ...
      ordersSnapshot.docs.forEach(doc => {
        ...
        // Hem webhook hem sync verisi olan siparişleri kontrol et
        if (data.lastWebhookAt && data.lastSyncAt) {
          const webhookAt = data.lastWebhookAt.toDate ? data.lastWebhookAt.toDate() : new Date(data.lastWebhookAt);
          const syncAt = data.lastSyncAt.toDate ? data.lastSyncAt.toDate() : new Date(data.lastSyncAt);
      ...
      return {
        ...
        conflicts: conflicts.slice(0, 50), // İlk 50 conflict'i döndür
      };
    } catch (error) {
  ...
  /**
   * Webhook kayıtlarının temizleme önizlemesi - silinecek webhook kayıt sayılarını hesapla
   * @param {string} integrationId - Integration ID
   * @returns {Object} Silinecek webhook kayıt sayıları
   */
  async getWebhookCleanupPreview(integrationId) {
    try {
      ...
      // 1. Deleted Webhook records sayısı
      const webhooksSnapshot = await adminFirestore
        .collection(SHOPIFY_WEBHOOKS_COLLECTION)
        .where("integrationId", "==", integrationId)
        .where("status", "==", "deleted")
      ...
      // 2. Webhook Events sayısı (shop domain ile)
  ...
  /**
   * 🧹 Duplicate webhook'ları temizle (aynı topic için birden fazla webhook)
   * Her topic için sadece en yeni olanı bırakır, eski olanları siler
   */
  async cleanupDuplicateWebhooks(integrationId) {
    try {
      logger.info(`Starting duplicate webhook cleanup for integration ${integrationId}`);
      
      // 1. Shopify'dan tüm webhook'ları çek
      ...
      // 3. Her topic için duplicate'leri tespit et
      const toDelete = [];
      const toKeep = [];
      ...
      for (const [topic, webhooks] of Object.entries(groupedByTopic)) {
        if (webhooks.length > 1) {
          ...
          // En yeni olanı bırak (created_at'e göre sırala)
  ...
  async processWebhookEvent(topic, data, headers) {
    try {
      ...
      // 🔒 IDEMPOTENCY CHECK - Daha önce işlendi mi?
      ...
      // ✅ İşlem başarılı - Webhook ID'yi kaydet (idempotency)
  ...
  /**
   * Webhook duplicate kontrolü - Idempotency
   * @returns {boolean} true ise daha önce işlenmiş
   */
  async checkWebhookDuplicate(webhookId, topic, shopifyId) {
    try {
      if (!webhookId) {
        logger.warn("No webhook ID provided for duplicate check");
        return false; // ID yoksa duplicate kontrolü yapamayız
      }
      ...
      // Hata durumunda false döndür - işlem devam etsin
      return false;
    }
  }

  /**
   * Webhook'u işlenmiş olarak işaretle - Idempotency tracking
   */
    ...
    try {
      ...
      await adminFirestore
        ...
        .add({
          ...
          // 30 gün sonra otomatik sil (Firestore TTL ile)
      ...
      // Bu hata critical değil - devam et
    }
  }
  ...
  async processOrderWebhook(orderData, topic, headers) {
    try {
      ...
      // Siparişi webhook kaynağı ile kaydet/güncelle
      ...
      // Eğer webhook ignore edildiyse log'la
  ...
  async processAppUninstalled(headers) {
    try {
      ...
      // Sadece webhook kayıtlarını temizle (sipariş/müşteri verileri korunur)
