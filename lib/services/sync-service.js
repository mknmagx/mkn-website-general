 ...
 * Bu servis tüm değişiklikleri otomatik olarak ilgili koleksiyonlara yansıtır
 */
...
import { ROLE_PERMISSIONS } from "./admin-permissions-service";

/**
 * Permission değiştiğinde etkilenen tüm rolleri ve kullanıcıları günceller
 */
export const syncPermissionChanges = async (permissionId, action, permissionData = null) => {
  try {
    console.log(`[SYNC] Permission ${action} started for: ${permissionId}`);
    ...
    console.log(`[SYNC] Permission sync completed. Updated roles: ${updatedRoles.length}, Users will be updated automatically`);
 ...
 */
export const syncRoleChanges = async (roleId, action, newPermissions = null) => {
  try {
    console.log(`[SYNC] Role ${action} started for: ${roleId}`);
    ...
    console.log(`[SYNC] Role sync completed. Updated users: ${updatedUsers}`);
 ...
 */
export const syncUsersWithRole = async (roleId, newPermissions, batch = null) => {
  try {
...
export const syncUserRoleChange = async (userId, newRole, oldRole = null) => {
  try {
    console.log(`[SYNC] User role change started for: ${userId}, new role: ${newRole}`);
    
    // Yeni rolün permission'larını al
    ...
    console.log(`[SYNC] User role change completed. New permissions count: ${newPermissions.length}`);
...
export const validateAndFixPermissionConsistency = async () => {
  try {
    console.log("[SYNC] Starting system-wide permission consistency check...");
    ...
    console.log(`[SYNC] Consistency check completed. Fixed users: ${fixedUsers}, Fixed roles: ${fixedRoles}`);
...
export const syncBulkChanges = async (changes) => {
  try {
    ...
    console.log(`[SYNC] Bulk sync completed. Processed ${totalUpdates} changes`);
