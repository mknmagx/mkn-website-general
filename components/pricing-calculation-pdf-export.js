...
const PricingCalculationPDFExport = ({ calculation, children, fileName }) => {
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      link.download =
        fileName ||
        `maliyet-hesaplama-${calculation.productName?.replace(/[^a-z0-9]/gi, "_") || "urun"}.pdf`;
      ...
      console.log("Pricing calculation PDF download completed successfully");
      setDialogOpen(false);
    } catch (err) {
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Maliyet hesaplama PDF'i için özelleştirme seçeneklerini belirleyin
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-4">
            {/* Show Cost Details Toggle */}
            <div className="flex items-center justify-between space-x-2">
              <Label htmlFor="costDetails" className="flex flex-col space-y-1">
                <span className="font-semibold text-gray-900">
                  Maliyet Detayları
                </span>
                <span className="font-normal text-xs text-gray-600">
                  Maliyetleri göster / sadece satış fiyatı göster
                </span>
              </Label>
              <Switch
                id="costDetails"
