...
export default function EditDeliveryPage() {
  ...
  // Hooks
  const { delivery, loading: fetchLoading, error } = useDelivery(deliveryId);
  const { updateDelivery, loading: updateLoading } = useUpdateDelivery();
  const {
  ...
  useEffect(() => {
    if (delivery && delivery.id) {
      // Force update form data with delivery values
      setFormData((prev) => ({
        type: delivery.type || "",
        companyInfo: {
          companyName: delivery.companyInfo?.companyName || "",
          contactPerson: delivery.companyInfo?.contactPerson || "",
          phone: delivery.companyInfo?.phone || "",
          email: delivery.companyInfo?.email || "",
          address: delivery.companyInfo?.address || "",
        },
        deliveryAddress: {
          address: delivery.deliveryAddress?.address || "",
          city: delivery.deliveryAddress?.city || "",
          district: delivery.deliveryAddress?.district || "",
      ...
      // Set company search term with current company name
      if (delivery.companyInfo?.companyName) {
        setCompanySearchTerm(delivery.companyInfo.companyName);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validItems = formData.items.filter(
        (item) => item.productName.trim() && item.quantity
      );
      ...
      const result = await updateDelivery(deliveryId, updateData);

      if (result.success) {
        toast({
          title: "Başarılı",
          description: "İrsaliye güncellendi",
        });
        router.push(`/admin/deliveries/${deliveryId}`);
      } else {
        toast({
          title: "Hata",
          description: result.error || "İrsaliye güncellenemedi",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Hata",
        description: "İrsaliye güncellenirken bir hata oluştu",
        variant: "destructive",
      });
    }
  };
  ...
  // Loading state
  if (authLoading || fetchLoading || !delivery) {
    return (
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600">
              İrsaliye düzenlemek için gerekli izinlere sahip değilsiniz.
            </p>
          </div>
        </div>
      }
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-6">
            <Button
              variant="outline"
              onClick={() => router.push(`/admin/deliveries/${deliveryId}`)}
            >
              <ArrowLeft className="h-4 w-4 mr-2" />
              Geri
            </Button>
            <div>
              ...
              <p className="text-gray-600 mt-1">
                İrsaliye #{delivery.deliveryNumber} - Bilgileri güncelleyin
              </p>
            </div>
          </div>
              ...
              <CardContent>
                <div className="grid gap-4">
                  <div>
                    ...
                    <Select
                      key={`delivery-type-${delivery?.id}-${formData.type}`}
                      value={formData.type}
                      onValueChange={(value) =>
                  ...
                  <div className="flex gap-4 mt-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          handleCompanyModeChange(e.target.value)
                        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      disabled={
                        companySelectionMode === "select" && selectedCompany
                      }
                    />
